...
<div class="members">
    ...
    <div class="members__column__content">
        <section class="c-box">
        <h3 class="c-box__hd">{{ __('customer.メールマガジン設定') }}</h3>
        ...
        <div class="c-box__content">
            ...
            <div class="l-column--row__label">{{ __('customer.登録中のメールアドレス') }}</div>
            <div class="l-column--row__content">{{$email}}</div>
            </div>
            <!--/.l-column l-column--row-->
            <hr class="c-hr">
            <div class="l-column l-column--row l-column--row--wide l-column--sp-single">
            <div class="l-column--row__label">fanaRtic{{ __('customer.メールマガジン') }}
                <p class="c-text--note">{{ __('customer.セール情報やショップオープンなど最新情報を配信') }}</p>
            </div>
            <div class="l-column--row__content">
                <label class="c-form__radio"><input type="radio" name="status" value="1"
                    @if($mmg->magazine_status == 1)
                        checked
                    @endif><i></i>{{ __('customer.購読') }}</label>
                <label class="c-form__radio c-text--attention"><input type="radio" name="status" value="0"
                    @if($mmg->magazine_status == 0)
                        checked
                    @endif><i></i>{{ __('customer.未購読') }}</label></div>
            </div>
            <!--/.l-column l-column--row-->
            <hr class="c-hr">
            <div class="l-button"><button class="c-button c-button--submit">{{ __('customer.購読内容を変更する') }}</button></div>
            <!--/.l-button-->
        </div>
        {{ Form::close() }}
        <!--/.c-box__content-->
