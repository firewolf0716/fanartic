...
<div class="">
                    ...
                    <table id="datatable" class="table table-striped table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th style="text-align:center">デザイナー</th>
                        ...
                        @foreach($designers as $designer)  
                            ...
                            <tr>
                                <td>{{$index}}</td>
                                <td width="0%" hidden>{{$designer->brand_id}}</td>
                                <td style="text-align:center">{{$designer->designer}}</td>
                                <td style="text-align:center">{{$designer->period}}</td>
                                <td style="text-align:center">{{$designer->birthplace}}</td>
                                <td style="text-align:center">
                                    <img style="height:20px;" src="{{url('images/designer/'.$designer->figure)}}">
                                </td>
                                <td style="text-align:center">
                                    <a href="{{ url('admin/designer/edit/'.$designer->designer_id) }}"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a>
                                    <a href="#">
                                        <span class="glyphicon glyphicon-trash" onclick="deleteConfirm( {{$designer->designer_id}} )" aria-hidden="true"></span>
                                    </a>
                                </td>
                            </tr>
                        @endforeach
    ...
    <script>
		function deleteConfirm(brand_id) {
			...
			window.location = "{{ url('admin/designer/delete') }}" + "/" + brand_id;
		}
	</script>
