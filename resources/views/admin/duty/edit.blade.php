...
<div class="">
    <div class="page-title">
        <div class="title_left" style="margin-Bottom:20px">
            <h3>{{ __('admin.グループ名前') }}</h3>
        </div>
    </div>
    <div class="clearfix"></div>
    <div class="col-md-12 col-sm-12 col-xs-12">
        {!! Form::open(array('id' => 'form_add','url'=>'admin/duty/editpost','class'=>'form-horizontal','enctype'=>'multipart/form-data', 'accept-charset' => 'UTF-8', 'novalidate')) !!}
        {{ Form::hidden('id', $duty->id)}}
        <div class="x_panel">
            <div class="x_title">
                <h4>{{ __('admin.グループ編集') }}</h4>
                <div class="clearfix"></div>
            </div>
            <div class="x_content">
                <div class="form-group">
                    ...
                    <div class="col-md-4 col-sm-6 col-xs-12">
                        <input type="text" id="duty_name" name="duty_name" required="required" class="form-control col-md-7 col-xs-12" value="{{$duty->name}}" placeholder="{{ __('admin.サイズの名前を日本語で入力してください') }}">
                    </div>
                </div>
                <div class="form-group">
                    <label class="control-label col-md-3 col-sm-3 col-xs-12">{{ __('admin.関税') }}(%)<span class="required">*</span></label>
                    <div class="col-md-4 col-sm-6 col-xs-12">
                        <input type="text" id="group_tariff" name="group_tariff" required="required" class="form-control col-md-7 col-xs-12" value="{{$duty->num}}" placeholder="{{ __('admin.サイズの名前を英語で入力してください') }}">
                    </div>
                </div>
                <div class="form-group">
                    <label class="control-label col-md-3 col-sm-3 col-xs-12">{{ __('admin.登録日時') }}</label>
                    <div class="col-md-4 col-sm-6 col-xs-12">
                        <input type="text" id="created_at" name="created_at" class="form-control col-md-7 col-xs-12" readonly value="{{$duty->created_at}}">
                    </div>
                </div>
                <div class="form-group">
                    <label class="control-label col-md-3 col-sm-3 col-xs-12">{{ __('admin.変更日時') }}</label>
                    <div class="col-md-4 col-sm-6 col-xs-12">
                        <input type="text" id="updated_at" name="updated_at" class="form-control col-md-7 col-xs-12" readonly value="{{$duty->updated_at}}">
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    $('#btnReset').click(function() {
        ...
        $('#updated_at').val('{{$duty->updated_at}}');
    });
