...
@section('title', 'FanaRtic Edit Notify Merchant')
...
<div class="">
    <div class="page-title">
        <div class="title_left" style="margin-Bottom:20px">
            <h3>{{ __('admin.通知を編集（商人)') }}</h3>
        </div>
    </div>
    <div class="clearfix"></div>
    <div class="col-md-12 col-sm-12 col-xs-12">
        {!! Form::open(array('id' => 'form_add','url'=>'admin/notifymerchant/editpost','class'=>'form-horizontal','enctype'=>'multipart/form-data', 'accept-charset' => 'UTF-8', 'novalidate')) !!}
        {{ Form::hidden('notify_id', $notify->notify_id)}}
        <div class="x_panel">
            <div class="x_title">
                <h4>{{ __('admin.通知を編集（商人)') }}</h4>
                <div class="clearfix"></div>
            </div>
            <div class="x_content">
                <div class="form-group">
                    ...
                    <div class="col-md-4 col-sm-6 col-xs-12">
                        ...
                        @foreach($merchants as $merchant)
                            <?php $selected = ''; ?>
                            @foreach($linkedMerchants as $linkedMerchant)
                                @if($merchant->merchant_id == $linkedMerchant)
                                    <?php $selected = ' selected'; ?>
                                    @break
                                @endif
                            @endforeach

                            <option value="{{$merchant->merchant_id}}"{{$selected}}>{{$merchant->name}}</option>
                        @endforeach
                        </select>
                    </div>
                </div>
