...
@section('title', 'FanaRtic List Notify Merchant')

@section('content')
<div class="">
    <div class="page-title">
        <div class="title_left" style="margin-Bottom:20px">
            <h3>{{ __('admin.リスト通知 (商人)') }}</h3>
        </div>
    </div>
                    ...
                    <table id="datatable" class="table table-striped table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th style="text-align:center">{{ __('admin.メモ') }}</th>
                        ...
                        <?php $index += 1; ?>
                            <tr>
                                ...
                                <td style="text-align:center">
                                    <a href="{{ url('admin/notifymerchant/edit/'.$notify->notify_id) }}"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a>
                                    <a href="#"><span class="glyphicon glyphicon-trash" onclick="deleteConfirm({{$notify->notify_id}})" aria-hidden="true"></span></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
    ...
    <script>
		function deleteConfirm(notify_id) {
			var answer = confirm('{{ __('admin.本当に削除しますか?') }}');
            if(!answer){
                return;
            }
			window.location = "{{ url('admin/notifymerchant/delete') }}" + "/" + notify_id;
		}
	</script>
