...
<div class="">
                ...
                <div class="x_content">
                    <table id="datatable" class="table table-striped table-bordered">
                        ...
                        @foreach($newses as $news)  
                            ...
                            <tr>
                                <td>{{$index}}</td>
                                <td width="0%" hidden>{{$news->b_id}}</td>
                                <td style="text-align:center">{{$news->b_name}}</td>
                                <td style="text-align:center">{{$news->date}}</td>
                                <td style="text-align:center">
                                    <img style="height:20px;" src="{{url('./images/news/'.$news->figure)}}">
                                </td>
                                <td style="text-align:center">
                                    <a href="{{ url('admin/news/edit/'.$news->bn_id) }}"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a>
                                    <a href="#">
                                        <span class="glyphicon glyphicon-trash" onclick="deleteConfirm({{$news->bn_id}})" aria-hidden="true"></span>
                                    </a>
                                </td>
                            </tr>
                        @endforeach
    ...
    <script>
		function deleteConfirm(brand_id) {
			...
			window.location = "{{ url('admin/news/delete') }}" + "/" + brand_id;
		}
	</script>
