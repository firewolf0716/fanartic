...
<div class="">
                ...
                <div class="x_content">
                    <table id="datatable" class="table table-striped table-bordered">
                        ...
                        @foreach($histories as $history)  
                            ...
                            <tr>
                                <td>{{$index}}</td>
                                <td width="0%" hidden>{{$history->b_id}}</td>
                                <td style="text-align:center">{{$history->b_name}}</td>
                                <td style="text-align:center">{{$history->year}}</td>
                                <td style="text-align:center">{{$history->head}}</td>
                                <td style="text-align:center">
                                    <img style="height:20px;" src="{{url('./images/history/'.$history->figure)}}">
                                </td>
                                <td style="text-align:center">
                                    <a href="{{ url('admin/history/edit/'.$history->bh_id) }}"><span class="glyphicon glyphicon-edit" aria-hidden="true"></span></a>
                                    <a href="#">
                                        <span class="glyphicon glyphicon-trash" onclick="deleteConfirm({{$history->bh_id}})" aria-hidden="true"></span>
                                    </a>
                                </td>
                            </tr>
                        @endforeach
    ...
    <script>
		function deleteConfirm(brand_id) {
			...
			window.location = "{{ url('admin/history/delete') }}" + "/" + brand_id;
		}
	</script>
