...
class MerchantShippingController extends Controller
{
    public function merchant_shipping_add() {
        ...
        return view('merchant.shipping.add')->with('brands', $brands);
    }
    ...
    public function merchant_shipping_addpost() {
        ...
        $entry =  array(
            ...
            'shipping_create' => Input::get('shipping_create'),
            'shipping_update' => Input::get('shipping_update')
        );

        if (Input::get('shipping_limit_date_set') == 0) {
            $entry['shipping_limit_date'] = Input::get('shipping_limit_date');
        } 

        if (Input::get('shipping_limit_duration_set') == 0) {
            $entry['shipping_limit_duration'] = Input::get('shipping_limit_duration');
        }

        $shippingID = MerchantsShipping::addMerchantShipping($entry);
